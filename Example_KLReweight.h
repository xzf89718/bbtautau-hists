...
void Plots_KLReweigt()
{      
        std::vector <std::string> list_klambda = {};
...
//
        ...
        for (VariableInfo* v : *(vs->content()))
        {
               ...
               ps->add("hhttbbKL1p0_2tag2pjet_0ptv_LL_SS_mHH",         "data",                         eProcessType::DATA,     eProcess::DATA,         "data",                         kBlack);
               ...
               //  //  ps->add("hhttbbKL1p0",  "HH (#kappa_{#lambda}=1.0)",    eProcessType::SIG,      eProcess::HH,           "HH (#kappa_{#lambda}=1.0)",    kRed);
                ps->add("hhttbbKL5p0from1p0",  "HH (#kappa_{#lambda}=5.0)",    eProcessType::SIG,      eProcess::HH,           "HH (#kappa_{#lambda}=5.0)",    kRed);
               ...
               //ps->add("hhttbb", "HH -> #tau#taubb", eProcessType::SIG, eProcess::HH, "HH -> #tau#taubb", kRed);
                ...
                c->load("/home/zifeng/HistCxAOD/KLReweight/KLReweight_py8.root", "Preselection");
                c->updateHistogramPtr(rs->content()->front(), v);
                DrawStackTool* ds = new DrawStackTool(info);
                ds->output_path = "/home/zifeng/HistCxAOD/KLReweigt/plots";
                if (ds->check(c))
                {
                        ds->manipulate(c);
                        ds->paint(c);
